...
if (isset($_GET['accept_quote'], $_GET['url_key'])) {
    ...
    if (mysqli_num_rows($sql) == 1) {
        ...
        if (!empty($config_smtp_host) && !empty($config_quote_notification_email)) {
            ...
            $body = "Hello, <br><br>This is a notification that a quote has been accepted in ITFlow. <br><br>Client: $client_name<br>Quote: <a href=\'https://$config_base_url/quote.php?quote_id=$quote_id\'>$quote_prefix$quote_number</a><br>$footer<br>$config_quote_from_email";
...
if (isset($_GET['decline_quote'], $_GET['url_key'])) {
    ...
    if (mysqli_num_rows($sql) == 1) {
        ...
        if (!empty($config_smtp_host) && !empty($config_quote_notification_email)) {
            ...
            $body = "Hello, <br><br>This is a notification that a quote has been declined in ITFlow. <br><br>Client: $client_name<br>Quote: <a href=\'https://$config_base_url/quote.php?quote_id=$quote_id\'>$quote_prefix$quote_number</a><br>$footer<br>$config_quote_from_email";
...
if (isset($_GET['export_quote_pdf'])) {
    ...
    if (mysqli_num_rows($sql) == 1) {
        ...
        $sql = mysqli_query($mysqli, "SELECT company_address, company_city, company_country, company_email, settings.company_id,
            company_locale, company_logo, company_legal_name, company_phone, company_phone_country_code,
        ...
        $company_name = escapeHtml($row['company_legal_name']);
...
if (isset($_GET['export_invoice_pdf'])) {
    ...
    if (mysqli_num_rows($sql) == 1) {
        ...
        $sql = mysqli_query($mysqli, "SELECT company_address, company_city, company_country, company_email, company_id, company_locale,
            company_logo, company_legal_name, company_phone, company_phone_country_code, company_state,
